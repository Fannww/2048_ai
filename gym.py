...
def step(grids, actions):
    ...
    scores[right_mask] = (right_out * rxm_mask).sum(dim=(1,2)).int()
    rmask = right_out != 0
    ...
    um_mask = torch.cat([um_mask, torch.full((grids[up_mask].size(0), 1, 4), 0, dtype=torch.bool, device=device)], dim=1) if um_mask.size(0) != 0 else torch.full((1, 4, 4), 0, device=device) #this will make it for up and i can just flip it for down 
    ...
    scores[up_mask] = (up_out * um_mask).sum(dim=(1,2)).int()
    umask = up_out != 0
    ...
    dm_mask = torch.cat([torch.full((grids[down_mask].size(0), 1, 4), 0, device=device, dtype=torch.bool), dm_mask], dim=1) if dm_mask.size(0) != 0 else torch.full((1, 4, 4), 0, device=device)
    ...
    scores[down_mask] = (down_out * dm_mask).sum(dim=(1,2)).int()
    dmask = down_out != 0
